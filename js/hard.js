(function() {
  ...
  const imageOptions = [
    'assets/hard/abstract-1.jpg',
    'assets/hard/abstract-2.jpg',
    'assets/hard/abstract-3.jpg',
    'assets/hard/abstract-4.jpg',
    'assets/hard/abstract-5.jpg'
  ];
  ...
  // Board border width (must match CSS)
  const BOARD_BORDER = 8;
  const BOARD_BORDER_RADIUS = 20;
  ...
  function startBackgroundMusic() {
    if (window.GameSettings && window.GameSettings.startMusic) {
      ...
      console.log('Background music started from Hard page');
    }
  }

  // ================= SETTINGS INTEGRATION =================
  function applyCurrentSettings() {
    console.log('Applying settings to Hard page');
  ...
  function showHint() {
    ...
    if (wrapper && wrapper.parentNode === trayEl) {
      ...
      const boardOffset = BOARD_BORDER;
      
      const targetLeft = hintPiece.correctX + boardOffset;
      const targetTop = hintPiece.correctY + boardOffset;
      ...
      boardHighlight.style.left = targetLeft + 'px';
      boardHighlight.style.top = targetTop + 'px';
      boardHighlight.style.width = pieceSize + 'px';
      boardHighlight.style.height = pieceSize + 'px';
      boardHighlight.style.borderRadius = '10px';
      boardHighlight.style.pointerEvents = 'none';
      boardHighlight.style.zIndex = '100';
      boardHighlight.style.border = '3px solid #ffd700';
      boardHighlight.style.backgroundColor = 'rgba(255, 215, 0, 0.15)';
      boardHighlight.style.boxSizing = 'border-box';
      boardHighlight.style.animation = 'hintPulse 0.8s ease-in-out infinite';
      
      boardEl.appendChild(boardHighlight);
      currentHintHighlight = boardHighlight;
      
      // Scroll the piece into view
  ...
  function addBoardGrid() {
    ...
    style.textContent = `
      #board {
        position: relative;
        background: #fffcf0;
        border: ${BOARD_BORDER}px solid #faedcd !important;
        border-radius: ${BOARD_BORDER_RADIUS}px !important;
        overflow: hidden;
      }
      #board::after {
        content: '';
  ...
  function saveGameResult() {
    if (checkWinCondition() && window.StatisticsManager) {
      window.StatisticsManager.addGameResult('hard', timerSeconds, moveCount, true);
    }
  }
    ...
    messageDiv.innerHTML = `
        <div class="completion-content">
            ...
            <div class="popup-buttons">
                ...
                <button onclick="playAgain()" class="popup-btn play-again-btn">🔄 Play Again</button>
            </div>
        </div>
    `;
  ...
  function checkWin() {
    if (checkWinCondition() && !gameCompleted) {
      gameCompleted = true;
      stopTimer();
      if (msgEl) msgEl.textContent = '✨ PERFECT! Hard puzzle complete! ✨';
  ...
  function drawPiece(r, c) {
    ...
    if (s.top === 0) {
      ...
      const bezierOffset = Math.min(4, Math.max(2, Math.floor(knob * 0.6)));
      ctx.lineTo(pieceSize / 3, 0);
      ctx.bezierCurveTo(
        pieceSize / 3 + bezierOffset, -dir * knob * 1.1,
        2 * pieceSize / 3 - bezierOffset, -dir * knob * 1.1,
        2 * pieceSize / 3, 0
      );
      ctx.lineTo(pieceSize, 0);
    }
    ...
    if (s.right === 0) {
      ...
      ctx.bezierCurveTo(
        pieceSize + dir * knob * 1.1, pieceSize / 3 + bezierOffset,
        pieceSize + dir * knob * 1.1, 2 * pieceSize / 3 - bezierOffset,
    ...
    if (s.bottom === 0) {
      ...
      ctx.bezierCurveTo(
        2 * pieceSize / 3 - bezierOffset, pieceSize + dir * knob * 1.1,
        pieceSize / 3 + bezierOffset, pieceSize + dir * knob * 1.1,
    ...
    if (s.left === 0) {
      ...
      ctx.bezierCurveTo(
        -dir * knob * 1.1, 2 * pieceSize / 3 - bezierOffset,
        -dir * knob * 1.1, pieceSize / 3 + bezierOffset,
        0, pieceSize / 3
      );
      ctx.lineTo(0, 0);
    }
    ...
    if (img.complete && img.naturalWidth && img.naturalWidth > 0) {
      ...
      ctx.font = `bold ${Math.floor(pieceSize * 0.35)}px Arial`;
  ...
  if (boardEl) {
    boardEl.style.border = `${BOARD_BORDER}px solid #faedcd`;
    boardEl.style.borderRadius = `${BOARD_BORDER_RADIUS}px`;
  }
